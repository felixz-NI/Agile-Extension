...
(function () {
    ...
    // --- Capture form data via page-context interception ---
    (function injectPageHooks() {
        ...
        (function() {
            ...
            var _capture = null;
            // After a capture resolves we keep swallowing navigations for a
            // short grace period. openFile() is often async (it does a server
            // round-trip before finally calling form.submit()/window.open()),
            // so the *real* navigation can fire AFTER we've already captured.
            // Without this guard that trailing call leaks through and the file
            // downloads in the background.
            var _blockUntil = 0;
            var GRACE_MS = 2500;

            function isArmed() {
                return _capture !== null || Date.now() < _blockUntil;
            }

            function settle(result) {
                if (_capture) {
                    var resolve = _capture.resolve;
                    _capture = null;
                    // Start the grace window so trailing async navigations from
                    // the same openFile() call are still suppressed.
                    _blockUntil = Date.now() + GRACE_MS;
                    resolve(result);
                }
            }

            window.__previewCapture = function(id) {
            ...
            // Allows the content script to explicitly end the grace window
            // (e.g. once the captured preview has been handled).
            window.__previewDisarm = function() {
                _capture = null;
                _blockUntil = 0;
            };
            ...
            window.open = function(url) {
                if (isArmed() && url) {
                    console.log('[Preview Pane] Blocked window.open (capture/grace):', url);
                    settle({ type: 'url', url: String(url), method: 'GET' });
                    return null;
                }
            ...
            window.__previewOrigSubmit = _origSubmit;
            HTMLFormElement.prototype.submit = function() {
                if (isArmed()) {
                    ...
                    settle({ type: 'form', action: action, method: method, fields: fields, target: this.target || '' });
                    return;
                }
            ...
            HTMLAnchorElement.prototype.click = function() {
                if (isArmed() && this.href && this.href !== 'javascript:void(0)' && this.href !== 'javascript:noaction()') {
                    console.log('[Preview Pane] Intercepted a.click():', this.href);
                    settle({ type: 'url', url: this.href, method: 'GET' });
                    return;
                }
                return _origClick.call(this);
            };

            console.log('[Preview Pane] Page-context hooks installed');
        })();
        `;
        document.documentElement.appendChild(script);
        script.remove();
        log('Page-context hooks injected');
    })();
    ...
    function captureFileUrl(link) {
        return new Promise((resolve) => {
            ...
            evalScript.textContent = `
                window.__previewCapture('${captureId}').then(function(result) {
                    document.dispatchEvent(new CustomEvent('__preview_url_captured', { detail: { id: '${captureId}', result: JSON.stringify(result) } }));
                });
            `;

            const handler = (e) => {
                if (e.detail && e.detail.id === captureId) {
                    document.removeEventListener('__preview_url_captured', handler);
                    const result = e.detail.result ? JSON.parse(e.detail.result) : null;
                    log('Captured result:', result);
                    resolve(result);
                }
            };
            document.addEventListener('__preview_url_captured', handler);
            ...
            try {
                ...
                if (onclickStr) {
                    ...
                    triggerScript.textContent = `(function() { ${onclickStr} })();`;
                    document.documentElement.appendChild(triggerScript);
                    triggerScript.remove();
                }
            } catch (err) {
                log('Error triggering onclick:', err);
                resolve(null);
            }
    ...
    function openInSidebar(captured, container, fileType) {
        ...
        // Submit via page context using the ORIGINAL (unhooked) form.submit
        const submitScript = document.createElement('script');
        submitScript.textContent = `
            (function() {
                var form = document.getElementById('__preview_submit_form');
                if (form && window.__previewOrigSubmit) {
                    window.__previewOrigSubmit.call(form);
                } else if (form) {
                    form.submit();
                }
            })();
        `;
        document.documentElement.appendChild(submitScript);
        submitScript.remove();
        form.remove();

        setTimeout(() => sessionStorage.removeItem(PREVIEW_FLAG), 2000);
    ...
    // Run JS in the background frame's PAGE context (so it can call Agile's own
    // displayObject / tab handlers, which the isolated content world cannot).
    function runInFrameContext(jsText) {
    ...
    // Resolves once the frame has booted Agile and exposes displayObject. We
    // probe by injecting a script that sets a marker attribute when the function
    // exists (readable from the content world via the DOM).
    function probeFrameReady() {
        return new Promise((resolve, reject) => {
            ...
            (function poll() {
                ...
                try { doc.documentElement.removeAttribute('data-tree-ready'); } catch (e) {}
                runInFrameContext("try{if(typeof displayObject==='function'){document.documentElement.setAttribute('data-tree-ready','1');}}catch(e){}");
                if (doc.documentElement.getAttribute('data-tree-ready') === '1') { resolve(true); return; }
    ...
    // unknown). Runs in page context so Agile's own handler fires.
    function clickFrameTab(re) {
        const src = re.source.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        runInFrameContext(
            "(function(){try{var re=new RegExp('" + src + "','" + re.flags + "');" +
            "var ls=document.querySelectorAll('#tabsDiv a,a,.tabConButton,.tabname,.tabConButtonSel,li a');" +
            "for(var i=0;i<ls.length;i++){var t=(ls[i].textContent||'').replace(/\\s+/g,' ').trim();" +
            "if(t&&re.test(t)){ls[i].click();return;}}}catch(e){}})();");
    }
    ...
    function loadRelations(classId, objId, tabId, captionRe, tableId, onPartial) {
        return ensureTreeFrame().then(() => {
            ...
            runInFrameContext("try{displayObject('ItemHandler','" + classId + "','" + objId + "','" + tab +
                "');}catch(e){console.error('[Part Tree] nav error',e);}");
    ...
    function navigateMainToObject(classId, objId) {
        ...
        runInPageContext("try{if(typeof displayObject==='function'){displayObject('ItemHandler','" +
            classId + "','" + objId + "','" + tab + "');}}catch(e){console.error('[Part Tree] open error',e);}");
    }
